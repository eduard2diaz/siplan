...
class PeriodActividadGeneralValidator extends ConstraintValidator
{
    private $registry;
    ...
    public function validate($value, Constraint $constraint)
    {
        ...
        if (!$constraint instanceof PeriodActividadGeneral) {
            throw new UnexpectedTypeException($constraint, __NAMESPACE__ . '\PeriodActividadGeneral');
        }
        ...
        if (!$id) {
            $cadena = "SELECT COUNT(r) FROM ".$entity." r  WHERE r.lugar= :".$constraint->place." AND ((:fechainicio <= r.".$constraint->from." AND :fechafin>=r.".$constraint->from.") OR (:fechainicio >= r.".$constraint->from." AND :fechafin<=r.".$constraint->to.") OR( :fechainicio<=r.".$constraint->to." AND :fechafin>=r.".$constraint->to."))";
        } else {
            $cadena = "SELECT COUNT(r) FROM ".$entity." r WHERE r.lugar= :".$constraint->place." AND r.id!= :id  AND((:fechainicio <= r.".$constraint->from." AND :fechafin>=r.".$constraint->from.") OR (:fechainicio >= r.".$constraint->from." AND :fechafin<=r.".$constraint->to.") OR( :fechainicio<=r.".$constraint->to." AND :fechafin>=r.".$constraint->to."))";
            $parameters['id'] = $id;
        }
